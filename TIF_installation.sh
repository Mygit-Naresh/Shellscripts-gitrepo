...
echo -e "\e[33m provide any of the options below \n install \n remove \n status \e[0m "
    ...
    read -p "please provide the package name to be removed " pkg_name
    sudo yum remove ${pkg_name} -y > /tmp/sucess.log 2> /tmp/failure.log
        if [ $? -eq 0 ]; then
        echo -e "\e[32m package removed sucessfully \e[0m"
        else
        echo -e "\e[31m package removal failure please check the provided package name \e[0m"
        fi
    elif [ $1 = "status" ]; then
